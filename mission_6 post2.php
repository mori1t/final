    ...
    if (!empty($_POST["upload"])) {
        ...
        try {
            ...
            if ($_FILES['upfile']['error'] === 4) {
                ...
                $sql = $pdo->prepare("INSERT INTO titest(username, bukatu, place, date) VALUES (:username, :bukatu, :place, :date)");
                ...
                $sql -> bindValue(':bukatu', $bukatu, PDO::PARAM_STR);
                ...
                $sql = $pdo->prepare("INSERT INTO titest(username, bukatu, place, file, date) VALUES (:username, :bukatu, :place, :file, :date)");
...
<?php
session_start();
echo '<h2>＜部活動、サークルイベント告知＞</h2>
<form action="" method="post" enctype="multipart/form-data">
<p><input type="hidden" value="'.$_SESSION["name"].'" name="username"></p>
<p>告知内容: <input type="text" name="bukatu" required></p>
...
$db[user] = "ユーザー名" ;
$db[pass] = "パスワード" ;
$abc = 'SELECT * FROM titest ORDER BY id DESC' ;
